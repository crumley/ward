...
test('a claim is recorded on the floor and read back from the record', async () => {
  ...
  const report = await claimRepository(ws, 1, 'demo');
  expect(report.outcome).toBe('claimed');
  expect(claimsOf((await resolveProject(ws, 1)).record)).toEqual(['demo']);
...
test('a repository is claimed by at most one open floor: the second claim MOVES it', async () => {
  await openProject(ws, 'first'); // floor 1
  await openProject(ws, 'second'); // floor 2
  await claimRepository(ws, 1, 'demo');
  const moved = await claimRepository(ws, 2, 'demo');

  expect(moved.outcome).toBe('moved');
  expect(moved.from).toBe(1);
  expect(claimsOf((await resolveProject(ws, 1)).record)).toEqual([]);
...
test('a moved claim names the open tasks that stay where they were opened', async () => {
  ...
  await openProject(ws, 'second'); // floor 2
  await claimRepository(ws, 1, 'demo');
  await openTask(ws, 'in-flight', { floor: 1, repositories: ['demo'] });
  await openTask(ws, 'also-here', { floor: 1, repositories: ['demo'] });
  await closeTask(ws, 'f1t1', 'abandoned'); // a closed task is not still in flight

  const moved = await claimRepository(ws, 2, 'demo');
  expect(moved.staying.map((task) => task.address)).toEqual(['f1t2']);
  expect(moved.staying[0]).toMatchObject({ slug: 'also-here', floor: 1 });
});

test('re-claiming where it already is converges and says so', async () => {
  await openProject(ws, 'first'); // floor 1
  await claimRepository(ws, 1, 'demo');
  expect((await claimRepository(ws, 1, 'demo')).outcome).toBe('satisfied');
  expect(claimsOf((await resolveProject(ws, 1)).record)).toEqual(['demo']);
});

test('an unregistered name is refused before anything is written', async () => {
  await openProject(ws, 'first'); // floor 1
  expect(claimRepository(ws, 1, 'nonesuch')).rejects.toThrow(
...
test('release drops the claim; releasing what was never held converges', async () => {
  await openProject(ws, 'first'); // floor 1
  await claimRepository(ws, 1, 'demo');
  expect((await releaseRepository(ws, 1, 'demo')).outcome).toBe('released');
  expect(claimsOf((await resolveProject(ws, 1)).record)).toEqual([]);
  expect((await releaseRepository(ws, 1, 'demo')).outcome).toBe('absent');
});
...
test('two claimants refuse and name both floors — --project resolves it', async () => {
  ...
  await openProject(ws, 'second'); // floor 2
  await claimRepository(ws, 1, 'demo');
  await claimRepository(ws, 2, 'other');
  expect(placeByAffinity(ws, ['demo', 'other'])).rejects.toThrow(
    /claimed by different floors — floor 1 claims demo \(first\); floor 2 claims other \(second\)/,
  );
});
...
test("a closed floor's claims are inert — they route nothing", async () => {
  await openProject(ws, 'first'); // floor 1
  await claimRepository(ws, 1, 'demo');
  await closeProject(ws, 1);
  expect(await placeByAffinity(ws, ['demo'])).toEqual({
    note: 'no floor claims demo — ward project claim FLOOR demo',
  });
  expect(claimRepository(ws, 1, 'demo')).rejects.toThrow(/floor 1 is closed/);
});

test('a task records what it touches, and a single record answers for worktree create', async () => {
  await openProject(ws, 'first'); // floor 1
  await claimRepository(ws, 1, 'demo');
  const one = await openTask(ws, 'single', { floor: 1, repositories: ['demo'] });
  expect(one.record.repositories).toEqual(['demo']);
  expect(recordedRepository(one)).toBe('demo');

  const several = await openTask(ws, 'several', { floor: 1, repositories: ['demo', 'other'] });
  expect(recordedRepository(several)).toBeUndefined(); // the caller names it
  const none = await openTask(ws, 'none', { floor: 1 });
...
test('doctor names a claim on an unregistered repository, with both remedies', async () => {
  ...
  expect(finding?.message).toContain("floor 1 (first) claims 'demo', which is not registered");
  expect(finding?.message).toContain('ward project release 1 demo');
