...
- **Design / implementation tests** — that the code does what *this* design says. They move and
  change with `design` + `src`. *E.g.* "the store writes the task-record front-matter schema this
...
- **Intent tests** — that the **durable constraints** hold regardless of how it is built (the
  invariants in [`../intent/`](../intent/)). These should survive a design swap. *E.g.* a
  containing scope's status is **derived**, never a stored field
  ([domain model](../intent/01-concepts/00-domain-model.md), §17); **resume is idempotent and
  closed stays closed** ([sessions](../intent/01-concepts/02-sessions-and-lifecycle.md)); **no
  local/personal/persona content crosses to a remote artifact**
  ([principles §4](../intent/00-foundation/01-principles.md),
  [remote provider](../intent/02-subsystems/06-remote-provider.md)). If a test must change because
  a *tool* changed, it was really a design test.
