...
> **Layer:** design — implementation plan. The _how_; may change; moves with `../src/` and
> `../test/`. **Status:** draft.
...
- **Language & schema approach.** Working assumption: **TypeScript with a Zod-style
  ...
  strongly typed _and_ runtime-validated — the language is replaceable, that property is not). The
...
- **Cross-cutting conventions.** The **no-lost-updates discipline** (derive → append → single-writer
  → serialize) is the default for every writer; deterministic, parseable output for the agent
  audience and readable output for the human (principle §8); errors and logs follow one shared
  convention (to settle).
- **How `design/` is organized.** Spine first (the settle-early decisions above), then one plan per
  seam and per cross-cutting mechanism — see [`README.md`](README.md).
