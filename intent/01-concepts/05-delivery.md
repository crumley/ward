...
defined in `00-work-hierarchy.md`; this slice owns its delivery.
...
- **The cardinal rule: never merge to main directly** — work reaches a main line only through a PR;
  ...
  outward/irreversible = gated — lives in `../00-foundation/01-principles.md` §18; this is its
  canonical instance.)_
...
- **Workflow policy: opinionated but evolvable** — the commit/merge opinions are shipped as a
  default and owned/evolved by the workspace. _(Uses the `../00-foundation/01-principles.md`
...
refresh/rebase. `03-artifacts.md` owns the artifact side of capture-on-close; this owns the boundary
crossing.
