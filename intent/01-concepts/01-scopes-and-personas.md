...
applies at every boundary — specialization (`../00-foundation/01-principles.md` §2) made concrete: depth
...
- **Exact-clone** — branch the origin's *actual* session state, so the fork begins knowing
  ...
  the harness can fork a session (`../02-subsystems/03-agent-harness.md`).
- **Brief (distilled)** — open a *fresh* session seeded by a **brief** that compacts the
  relevant context (`00-domain-model.md`). Lower fidelity but **harness-neutral** and more
...
The two axes (scope, working directory) are defined in
...
- **Fork mode first.** Which inheritance mode ships first — harness-neutral *distilled brief* or
  *exact-clone* (also tracked under [`../02-subsystems/03-agent-harness.md`](../02-subsystems/03-agent-harness.md)).
