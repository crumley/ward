...
every session is recorded with enough metadata to reconstruct it (`../00-foundation/01-principles.md` §16:
...
**resume** it and to **reflect** over it later (`04-reflection-and-evolution.md`). The handle
...
remote-work-item link without that link being its identity (`03-work-lifecycle.md`). **Why
the indirection:** it lets Ward span a mix of harnesses without assuming any one of them
(`05-context-loading.md`).
...
delegate-and-return pattern (`01-scopes-and-personas.md`) without senior scopes wasting
context on busy-waiting (mechanism: `../02-subsystems/02-messaging-coordination.md`).
...
reveals the threads beneath it. (Store is a *how* — `../02-subsystems/00-metadata-store.md`.)
...
- **Wait** lets one session block on another's completion; **wake** lets a session detach and
  ...
  indirection of pause and resume. (Mechanism: `../02-subsystems/02-messaging-coordination.md`.)
...
- **The session lifecycle** — open / close / resume / wake — and the **open ≠ running**
  distinction.
...
The **harness handle** is defined here as a recorded *attribute*; the contract a harness must
...
- **Wake across a reboot.** How a "wake me when X" request survives a reboot and is re-armed
  during recovery (mechanism in
