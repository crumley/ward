...
- **Idempotent lifecycle and actions.** Operations that resume, open, or wake a thread —
  and setup/teardown actions (`05-work-lifecycle.md`) — are safe to repeat. Repeating one
...
(`07-subsystem-seams.md`) with their durable choices recorded in `../how/`. **Why:** a
concept that cannot be stated without naming a tool has been modeled at the wrong level,
and will break when the tool is swapped.
...
audience** unless the caller declares itself an agent (`../how/cli-and-telemetry.md`).
...
primary source of compounding (§12, `06-reflection-and-evolution.md`).
...
catch an error at its root (`02-domain-model.md`, artifacts).
...
context is the concrete lever (`../how/context-loading.md`).
...
(`06-reflection-and-evolution.md`).
...
normal (`02-domain-model.md`) — and no concurrent writer may silently clobber another's
update. Three biases make this safe: **derive shared state rather than store it** (a parent's
status is a query over its children, not a maintained field — see `02-domain-model.md`),
...
restores from (§16). (Mechanism: `../how/metadata-and-schemas.md`.)
...
exists to prevent (the never-merge-to-main rule, `05-work-lifecycle.md`, is the canonical
