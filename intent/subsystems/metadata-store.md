...
> **Design:** `../../design/subsystems/metadata-store.md`. **Status:** placeholder skeleton.
...
the version stamp. The source of truth that survives reboots (`principles.md` §16).
...
- Aggregate status is **derived** from leaf records, not stored (`concepts/work-hierarchy.md`).
- Concurrent writes **do not lose updates** (`principles.md` §17) — via, in order of preference:
  derive-don't-store, append-over-rewrite, one-owner-per-record, serialize the rest.
- Legible to both audiences (`principles.md` §8) and git-versionable (`principles.md` §15).
