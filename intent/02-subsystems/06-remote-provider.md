...
> **Design:** `../../design/02-subsystems/06-remote-provider.md`. **Status:** placeholder skeleton.

## Responsibility

Be the remote side of the local↔remote boundary (`01-concepts/05-delivery.md`): the shared system
where work items and pull requests live.
...
- Accept outward-translated updates — the **privacy boundary is enforced _upstream_ of this seam**,
  so the provider receives already-sanitized content (`01-principles.md` §4).
- Report PR status so Ward can drive a task to completion.
- Outward posts/merges are **gated actions** (`01-principles.md` §18); the gate is upstream.
